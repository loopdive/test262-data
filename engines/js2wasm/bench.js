...
const call = (arg, name, params) => {
  const out = spawnSync('node', [RUNNER, arg, name, JSON.stringify({ ...params, target })], {
    encoding: 'utf8',
...
export default ({ name, sourcePath, subject, targetMs, rounds, maxReps }) => {
  const r = call(sourcePath, name, { subject, targetMs, rounds, maxReps });
